// NOTE: sourced from https://github.com/codemirror/view/blob/main/src/gutter.ts
...
import type { BlockInfo, EditorState, Extension, RangeCursor, ViewUpdate } from '@uiw/react-codemirror';
import {
  BlockType,
  combineConfig,
  Direction,
...
export const gutter = (config: GutterConfig): Extension => [gutters(), activeGutters.of({ ...defaults, ...config })];
...
const asArray = (val: any | readonly any[]) => (Array.isArray(val) ? val : [val]) as readonly any[];
...
class UpdateContext {
  ...
  line(view: EditorView, line: BlockInfo, extraMarkers: readonly GutterMarker[]) {
    ...
    const localMarkers = extraMarkers.length ? this.localMarkers.concat(extraMarkers) : this.localMarkers;
...
const lineNumberConfig = Facet.define<ILineNumberConfig, Required<ILineNumberConfig>>({
  combine: (values) =>
    combineConfig<Required<ILineNumberConfig>>(
      values,
      { formatNumber: String, domEventHandlers: {} },
      {
        domEventHandlers(a: Handlers, b: Handlers) {
          const result: Handlers = { ...a };
...
const formatNumber = (view: EditorView, number: number) => {
  return view.state.facet(lineNumberConfig).formatNumber(number, view.state);
};

// Modified block to handle line gutters and highlights to our use case.
const lineNumberGutter = activeGutters.compute([lineNumberConfig], (state) => ({
  class: 'cm-lineNumbers',
  renderEmptyElements: false,
  markers: (view: EditorView) => view.state.facet(lineNumberMarkers),
  lineMarker(view, line, others) {
    if (others.some((m) => m.toDOM)) return null;
    return new NumberMarker(formatNumber(view, view.state.doc.lineAt(line.from).number));
  },
  lineMarkerChange: (update) => update.startState.facet(lineNumberConfig) !== update.state.facet(lineNumberConfig),
  initialSpacer: (view: EditorView) => new NumberMarker(formatNumber(view, maxLineNumber(view.state.doc.lines))),
...
export const lineNumbers = (config: ILineNumberConfig = {}): Extension => [
...
const focusedLineGutterMarker = new (class extends GutterMarker {
  elementClass = 'cm-activeLineGutter--focused';
})();

const activeLineGutterHighlighter = gutterLineClass.compute(['selection'], (state: EditorState) => {
  const marks = [];
  let last = -1;
  for (const range of state.selection.ranges) {
    // turn off gutter highlighting for sele
    const linePos = state.doc.lineAt(range.head).from;
    if (range.empty && linePos > last) {
      last = linePos;
      marks.push(activeLineGutterMarker.range(linePos));
      marks.push(focusedLineGutterMarker.range(linePos));
    }
  }
  return RangeSet.of(marks);
});

export const highlightActiveLineGutter = () => activeLineGutterHighlighter;
