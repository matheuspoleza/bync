...
const lengthOfArrayUntilIntersect = (utheta: number, uline: ILine): number =>
...
const getBounds = (L: number): ILine[] => {
  const result: ILine[] = [];
...
const maxChromaForLH = (L: number, H: number): number => {
  ...
  const bounds: ILine[] = getBounds(L);
...
const yToL = (Y: number): number => (Y <= epsilon ? (Y / refY) * kappa : 116 * (Y / refY) ** (1.0 / 3.0) - 16);
const toLinear = (c: number): number => (c > 0.04045 ? ((c + 0.055) / (1 + 0.055)) ** 2.4 : c / 12.92);
